...
object Walmart extends App {
    ...
    */
    ...
    val walmartDF = spark.read.option("header", "true").option("inferSchema","true").csv("../dataset/walmart.csv")
    ...
    val samciTrosenjeZanimanja = walmartDF.filter($"Marital_Status" ===0)
    ...
    */
    ...
    val proizvodProdaja = walmartDF.groupBy("Product_ID", "Age", "City_Category")
    ...
    */
    ...
    val occupationProizvodDS = walmartDF.groupBy("Occupation", "Product_ID", "Gender")
                                    .agg(count("*") as "Amount")
    ...
    val occupationProductCategory = walmartDF.groupBy("Occupation", "Gender", "Product_Category")
    ...
    */
    ...
    val zivotnaDobSamci = walmartDF.filter($"Marital_Status" ===0)
    ...
    val muskarciTrosenje = walmartDF.filter($"Marital_Status" ===0 and $"Gender"==="M")
                            .groupBy("Age","City_Category").agg(avg("Purchase") as "Muskarci Potrosnja")
    
    val zeneTrosenje = walmartDF.filter($"Marital_Status" ===0 and $"Gender"==="F")
                            .groupBy("Age","City_Category")
