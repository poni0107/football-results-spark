error id: file:///D:/dev/workspaces/eclipse-workspace-2022/PDS-Spark/Scala/src/main/scala/Walmart.scala:_empty_/Walmart.walmartDS.
file:///D:/dev/workspaces/eclipse-workspace-2022/PDS-Spark/Scala/src/main/scala/Walmart.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol _empty_/Walmart.walmartDS.
...
1000003| P00193542|     M|26-35|        15|            A|                         3|             0|               1|   15227|
    ...
    */
    ...
    val walmartDS@@ = spark.read.option("header", "true").option("inferSchema","true").csv("../dataset/walmart.csv")
    ...
    val samciTrosenjeZanimanja = walmartDS.filter($"Marital_Status" ===0)
...
1004474|         7|            C|16639.941176470587|
    ...
    */
    ...
    // Zadatak 2 - Дати листу производа са информацијом у којем животном добу се највише купује неки производ и у којој категорији града.  

    import org.apache.spark.sql.expressions.Window

    val proizvodProdaja = walmartDS.groupBy("Product_ID", "Age", "City_Category")
...
Product_ID|  Age|City_Category|Max Prodaja|
 ...
 P00058042|26-35|            B|        225|
    ...
    */
    ...
    val occupationProizvodDS = walmartDS.groupBy("Occupation", "Product_ID", "Gender")
                                    .agg(count("*") as "Amount")
                                    .orderBy("Occupation", "Product_ID", "Gender")
    occupationProizvodDS.show()

    val topOccupationKupovina = 
        occupationProizvodDS.withColumn("Max Kupovina", max("Amount").over(Window.partitionBy("Occupation", "Gender")))
                            .filter($"Amount" === $"Max Kupovina")
                            .drop("Amount")
                            .orderBy("Occupation", "Product_ID", "Gender")
    ...
    topOccupationKupovina.withColumn("Gender", when($"Gender" === "F", "Zene").otherwise("Muskarci"))
                        .show()
  
    // -------

    val occupationProductCategory = walmartDS.groupBy("Occupation", "Gender", "Product_Category")
                                    ...
                                    .filter($"Category_Amount" === $"Max Category Kupovina")
                                    .drop("Category_Amount")
    
    val zad3fullResult = occupationProductCategory.join(topOccupationKupovina, Seq("Occupation", "Gender"))
...
Occupation|Gender|Product_Category|Max Category Kupovina|Product_ID|Max Kupovina|
    ...
    */
    ...
    val zivotnaDobSamci = walmartDS.filter($"Marital_Status" ===0)
    ...
    val muskarciTrosenje = walmartDS.filter($"Marital_Status" ===0 and $"Gender"==="M")
                            .groupBy("Age","City_Category").agg(avg("Purchase") as "Muskarci Potrosnja")
    
    val zeneTrosenje = walmartDS.filter($"Marital_Status" ===0 and $"Gender"==="F")
                            .groupBy("Age","City_Category")
